...
> **Tip 3:** "How would you describe the trade-off between Kafka and Kinesis?" — Kafka: open source, highly tunable, self-managed (or Confluent/MSK managed), unlimited retention, more ecosystem integrations. Kinesis: fully managed AWS service, integrates natively with AWS (Lambda, Firehose, Analytics), retention configurable up to 365 days (24h default; beyond 7 days costs extra), 1MB/sec write per shard. Choose Kinesis for: pure AWS shops wanting zero ops, simple pipelines. Choose Kafka for: cross-cloud portability, complex stream processing (Flink/ksqlDB), long retention requirements, or when Kafka skills already exist on the team.
